# Python 30 天学习计划 · 学习笔记与刷题记录

> 从零系统学习 Python：**每天一个主题，边学边敲、边学边刷**，30 天建立从基础语法到进阶特性的完整知识体系。

本仓库是 30 天学习计划的落地仓库，包含两部分：

- **学习笔记**（`学习/`）：11 个主题的知识速整笔记 + 可运行代码
- **刷题练习**（`刷题/`）：59 道基础入门题（5 大分类）+ Python100Days 39 题（对应「学习」11 个主题）+ 每日自主做题

本 README 汇总 30 天计划的整体安排与每日学习进度。

**当前进度：Day 1 / 30（2026-09-14）**

---

## 一、30 天学习计划

| 阶段 | 天数 | 日期 | 学习主题 | 对应目录 | 配套练习 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| 一、基础语法 | Day 1 | 09-14 | 基础语法知识 + 流程控制（变量 / 数据类型 / 进制 / 输入输出 / 运算符 / 分支 / 循环） | `学习/1` `学习/2` | python基础 9 题 | 已完成 |
| 一、基础语法 | Day 2 | 09-15 | 容器数据类型：字符串 str | `学习/3` | 丰富的数据类型（字符串部分） | 待开始 |
| 一、基础语法 | Day 3 | 09-16 | 容器数据类型：列表 list、元组 tuple | `学习/3` | 丰富的数据类型（列表 / 元组部分） | 待开始 |
| 一、基础语法 | Day 4 | 09-17 | 容器数据类型：集合 set、字典 dict | `学习/3` | 丰富的数据类型（集合 / 字典部分） | 待开始 |
| 一、基础语法 | Day 5 | 09-18 | 容器综合复习：四类容器对比 | `学习/3` | 丰富的数据类型 17 题 | 待开始 |
| 一、基础语法 | Day 6 | 09-19 | 函数基础：定义 / 调用 / 参数 / 返回值 / 说明文档 | `学习/4` | 神奇的函数（部分） | 待开始 |
| 一、基础语法 | Day 7 | 09-20 | 函数进阶：嵌套 / 作用域 / 递归 / 匿名函数 | `学习/4` | 神奇的函数 9 题 | 待开始 |
| 一、基础语法 | Day 8 | 09-21 | 文件操作：打开关闭 / 文件读写 / 常用函数 / 文件拷贝 | `学习/5` | — | 待开始 |
| 二、面向对象与异常 | Day 9 | 09-22 | 错误与异常：try-except / raise / 自定义异常 | `学习/7` | — | 待开始 |
| 二、面向对象与异常 | Day 10 | 09-23 | 模块与包：import / 包结构 / 模块使用 | `学习/8` | — | 待开始 |
| 二、面向对象与异常 | Day 11 | 09-24 | 标准模块实战：math / random 等 | `学习/8` | 标准模块 3 题 | 待开始 |
| 二、面向对象与异常 | Day 12 | 09-25 | 面向对象：类与对象 / 属性 / 方法 / `__init__` | `学习/6` | — | 待开始 |
| 二、面向对象与异常 | Day 13 | 09-26 | 面向对象：封装 | `学习/6` | — | 待开始 |
| 二、面向对象与异常 | Day 14 | 09-27 | 面向对象：继承 | `学习/6` | — | 待开始 |
| 二、面向对象与异常 | Day 15 | 09-28 | 面向对象：多态 + 综合练习 | `学习/6` | — | 待开始 |
| 三、高级主题 | Day 16 | 09-29 | Python 高级语法：浅拷贝与深拷贝 / 命名空间 / 作用域 | `学习/9` | — | 待开始 |
| 三、高级主题 | Day 17 | 09-30 | Python 高级语法：迭代器 / 生成器 | `学习/9` | — | 待开始 |
| 三、高级主题 | Day 18 | 10-01 | Python 高级语法：闭包 / 装饰器 | `学习/9` | — | 待开始 |
| 三、高级主题 | Day 19 | 10-02 | 进程与线程：多进程 / 多线程 / 对比 | `学习/10` | — | 待开始 |
| 三、高级主题 | Day 20 | 10-03 | 正则表达式：re 模块基础 | `学习/11` | — | 待开始 |
| 三、高级主题 | Day 21 | 10-04 | 综合实战：自主做题 | `刷题/自主做题` | P01 起每日 1 题 | 待开始 |
| 三、高级主题 | Day 22 | 10-05 | 综合实战：自主做题 | `刷题/自主做题` | 每日 1 题 | 待开始 |
| 四、巩固冲刺 | Day 23 | 10-06 | 刷题巩固：59 题查漏补缺 | `刷题` | — | 待开始 |
| 四、巩固冲刺 | Day 24 | 10-07 | 刷题巩固：流程控制 / 容器专项 | `刷题` | — | 待开始 |
| 四、巩固冲刺 | Day 25 | 10-08 | 刷题巩固：函数 / 模块专项 | `刷题` | — | 待开始 |
| 四、巩固冲刺 | Day 26 | 10-09 | 刷题巩固：自主做题 | `刷题/自主做题` | — | 待开始 |
| 四、巩固冲刺 | Day 27 | 10-10 | 小项目实战：命令行小工具 | — | — | 待开始 |
| 四、巩固冲刺 | Day 28 | 10-11 | 小项目实战：数据处理小脚本 | — | — | 待开始 |
| 四、巩固冲刺 | Day 29 | 10-12 | 错题回顾 + 知识体系梳理 | — | — | 待开始 |
| 四、巩固冲刺 | Day 30 | 10-13 | 总结复盘：输出 30 天学习总结 | — | — | 待开始 |

---

## 二、每日学习记录

> 每天学习后在这里追加一行，并在上方计划表中更新对应天数的状态。

| 日期 | 阶段 | 学习内容 | 练习 / 产出 |
| --- | --- | --- | --- |
| 09-11 | 准备 | 初始化 GitHub 仓库，搭建「学习 / 刷题」目录结构 | 编写 59 道基础入门题的题目要求与解答（5 大分类） |
| 09-12 | 准备 | 继续刷题巩固 | 自主写题 |
| 09-13 | 准备 | 容器数据类型：列表、字符串（提前试学） | [P01_List.py](学习/3%20容器数据类型/P01_List.py)、[P02_String.py](学习/3%20容器数据类型/P02_String.py) |
| 09-14 | Day 1 | 基础语法 + 流程控制复习（变量 / 类型 / 进制 / 运算符 / 分支 / 循环）；容器：元组、集合、字典；函数概念入门 | [day01_review.py](学习/1%20基础语法知识/day01_review.py)、[P03_Tuple.py](学习/3%20容器数据类型/P03_Tuple.py)、[P04_Set.py](学习/3%20容器数据类型/P04_Set.py)、[P05_Dict.py](学习/3%20容器数据类型/P05_Dict.py)、[P01_Concept.py](学习/4%20函数/P01_Concept.py)、[P01_1day.py](刷题/自主做题/P01_1day.py) |
| 09-15 | Day 2 | （待更新） | |

---

## 三、仓库结构

```
learning-notes-python/
├── 学习/                        # 学习笔记：11 个主题，按编号顺序学习
│   ├── 1 基础语法知识/           # 变量、数据类型、进制、输入输出、运算符
│   ├── 2 流程控制语句/           # 分支、循环、break / continue
│   ├── 3 容器数据类型/           # 字符串、列表、元组、集合、字典
│   ├── 4 函数/                  # 定义、参数、返回值、作用域、递归、匿名函数
│   ├── 5 文件操作/
│   ├── 6 面向对象/               # 类与对象、封装、继承、多态
│   ├── 7 错误与异常/
│   ├── 8 模块与包/
│   ├── 9 Python高级语法/         # 深浅拷贝、迭代器、生成器、闭包、装饰器
│   ├── 10 进程与线程/
│   └── 11 正则表达式/
├── 刷题/                        # 刷题练习
│   ├── 59道基础入门题/           # 五大分类（当前已收录 58 道）
│   │   ├── python基础/          # 9 题
│   │   ├── 丰富的数据类型/       # 17 题
│   │   ├── 标准模块/            # 3 题
│   │   ├── 流程控制/            # 20 题
│   │   └── 神奇的函数/          # 9 题
│   ├── Python100Days/            # 骆昊 Python-100-Days 提取（39 题，按「学习」11 主题对应）
│   └── 自主做题/                # 每日自主练习（P01_1day.py 起）
├── README.md                    # 本文件：30 天计划 + 每日学习记录
└── .gitignore
```

---

## 四、刷题清单

### 59 道基础入门题（已收录 58 道）

<details>
<summary>python基础（9 题）</summary>

- [AA制问题.py](刷题/59道基础入门题/python基础/AA制问题.py)
- [三个数的平均值.py](刷题/59道基础入门题/python基础/三个数的平均值.py)
- [三角形面积.py](刷题/59道基础入门题/python基础/三角形面积.py)
- [分转换为秒.py](刷题/59道基础入门题/python基础/分转换为秒.py)
- [利息问题.py](刷题/59道基础入门题/python基础/利息问题.py)
- [均分巧克力.py](刷题/59道基础入门题/python基础/均分巧克力.py)
- [按要求打印日期.py](刷题/59道基础入门题/python基础/按要求打印日期.py)
- [计算BMI.py](刷题/59道基础入门题/python基础/计算BMI.py)
- [计算利润或亏损.py](刷题/59道基础入门题/python基础/计算利润或亏损.py)
</details>

<details>
<summary>丰富的数据类型（17 题）</summary>

- [依次打印列表元素.py](刷题/59道基础入门题/丰富的数据类型/依次打印列表元素.py)
- [列表元素的积.py](刷题/59道基础入门题/丰富的数据类型/列表元素的积.py)
- [列表切片.py](刷题/59道基础入门题/丰富的数据类型/列表切片.py)
- [列表末尾添加元素.py](刷题/59道基础入门题/丰富的数据类型/列表末尾添加元素.py)
- [创建字典.py](刷题/59道基础入门题/丰富的数据类型/创建字典.py)
- [判断字符串首尾字符是否相等.py](刷题/59道基础入门题/丰富的数据类型/判断字符串首尾字符是否相等.py)
- [判断用户输入是否在字典的值中.py](刷题/59道基础入门题/丰富的数据类型/判断用户输入是否在字典的值中.py)
- [反转列表.py](刷题/59道基础入门题/丰富的数据类型/反转列表.py)
- [合并两个字典.py](刷题/59道基础入门题/丰富的数据类型/合并两个字典.py)
- [字符串是否相同(忽略大小写).py](刷题/59道基础入门题/丰富的数据类型/字符串是否相同(忽略大小写).py)
- [字符串替换.py](刷题/59道基础入门题/丰富的数据类型/字符串替换.py)
- [字符串连接.py](刷题/59道基础入门题/丰富的数据类型/字符串连接.py)
- [打印列表中的奇数.py](刷题/59道基础入门题/丰富的数据类型/打印列表中的奇数.py)
- [打印字典的值.py](刷题/59道基础入门题/丰富的数据类型/打印字典的值.py)
- [统计字符出现次数.py](刷题/59道基础入门题/丰富的数据类型/统计字符出现次数.py)
- [转换字符串到列表.py](刷题/59道基础入门题/丰富的数据类型/转换字符串到列表.py)
- [集合的合集.py](刷题/59道基础入门题/丰富的数据类型/集合的合集.py)
</details>

<details>
<summary>标准模块（3 题）</summary>

- [求一个数字的平方根.py](刷题/59道基础入门题/标准模块/求一个数字的平方根.py)
- [求用户输入值的四次幂.py](刷题/59道基础入门题/标准模块/求用户输入值的四次幂.py)
- [计算标准差.py](刷题/59道基础入门题/标准模块/计算标准差.py)
</details>

<details>
<summary>流程控制（20 题）</summary>

- [Fizz、Buzz 和 FizzBuzz.py](刷题/59道基础入门题/流程控制/Fizz、Buzz%20和%20FizzBuzz.py)
- [一个整数有多少位？.py](刷题/59道基础入门题/流程控制/一个整数有多少位？.py)
- [一个整数的所有因子.py](刷题/59道基础入门题/流程控制/一个整数的所有因子.py)
- [三数最小值.py](刷题/59道基础入门题/流程控制/三数最小值.py)
- [不要3的倍数.py](刷题/59道基础入门题/流程控制/不要3的倍数.py)
- [你具有投票资格么？.py](刷题/59道基础入门题/流程控制/你具有投票资格么？.py)
- [判断完全平方数.py](刷题/59道基础入门题/流程控制/判断完全平方数.py)
- [判断质数.py](刷题/59道基础入门题/流程控制/判断质数.py)
- [奇数还是偶数？.py](刷题/59道基础入门题/流程控制/奇数还是偶数？.py)
- [打印乘法表.py](刷题/59道基础入门题/流程控制/打印乘法表.py)
- [打印小于n的斐波那契数.py](刷题/59道基础入门题/流程控制/打印小于n的斐波那契数.py)
- [打印正整数.py](刷题/59道基础入门题/流程控制/打印正整数.py)
- [斐波那契数列.py](刷题/59道基础入门题/流程控制/斐波那契数列.py)
- [星期几？.py](刷题/59道基础入门题/流程控制/星期几？.py)
- [直到用户输入0.py](刷题/59道基础入门题/流程控制/直到用户输入0.py)
- [自然数求和.py](刷题/59道基础入门题/流程控制/自然数求和.py)
- [自行车还是汽车？.py](刷题/59道基础入门题/流程控制/自行车还是汽车？.py)
- [计算员工奖金.py](刷题/59道基础入门题/流程控制/计算员工奖金.py)
- [通过、失败、无效分数判断.py](刷题/59道基础入门题/流程控制/通过、失败、无效分数判断.py)
- [通过与失败.py](刷题/59道基础入门题/流程控制/通过与失败.py)
</details>

<details>
<summary>神奇的函数（9 题）</summary>

- [判断两次输入是否相等.py](刷题/59道基础入门题/神奇的函数/判断两次输入是否相等.py)
- [判断是否通过考试.py](刷题/59道基础入门题/神奇的函数/判断是否通过考试.py)
- [判断质数（使用函数）.py](刷题/59道基础入门题/神奇的函数/判断质数（使用函数）.py)
- [前N个自然数之和.py](刷题/59道基础入门题/神奇的函数/前N个自然数之和.py)
- [多次调用同一个函数.py](刷题/59道基础入门题/神奇的函数/多次调用同一个函数.py)
- [打印个人信息.py](刷题/59道基础入门题/神奇的函数/打印个人信息.py)
- [打印全名.py](刷题/59道基础入门题/神奇的函数/打印全名.py)
- [整数的幂.py](刷题/59道基础入门题/神奇的函数/整数的幂.py)
- [计算圆面积.py](刷题/59道基础入门题/神奇的函数/计算圆面积.py)
</details>

### Python100Days（39 题，对应「学习」11 个主题）

> 题目与示例提取自 [骆昊 Python-100-Days](https://github.com/jackfrued/Python-100-Days)（GPL-3.0），按「学习」目录的 11 个主题对应组织，边学边刷。

<details>
<summary>主题 1 · 基础语法（Day04 运算符 · 3 题）</summary>

- [华氏温度转摄氏温度.py](刷题/Python100Days/Day04_华氏温度转摄氏温度.py)
- [计算圆的周长和面积.py](刷题/Python100Days/Day04_计算圆的周长和面积.py)
- [判断闰年.py](刷题/Python100Days/Day04_判断闰年.py)
</details>

<details>
<summary>主题 2 · 流程控制（Day05 分支 / Day06 循环 / Day07 实战 · 11 题）</summary>

- [分段函数求值.py](刷题/Python100Days/Day05_分段函数求值.py)
- [百分制成绩转换成等级.py](刷题/Python100Days/Day05_百分制成绩转换成等级.py)
- [计算三角形的周长和面积.py](刷题/Python100Days/Day05_计算三角形的周长和面积.py)
- [判断素数.py](刷题/Python100Days/Day06_判断素数.py)
- [最大公约数.py](刷题/Python100Days/Day06_最大公约数.py)
- [猜数字游戏.py](刷题/Python100Days/Day06_猜数字游戏.py)
- [100以内的素数.py](刷题/Python100Days/Day07_100以内的素数.py)
- [斐波那契数列.py](刷题/Python100Days/Day07_斐波那契数列.py)
- [寻找水仙花数.py](刷题/Python100Days/Day07_寻找水仙花数.py)
- [百钱百鸡问题.py](刷题/Python100Days/Day07_百钱百鸡问题.py)
- [CRAPS赌博游戏.py](刷题/Python100Days/Day07_CRAPS赌博游戏.py)
</details>

<details>
<summary>主题 3 · 容器数据类型（Day09-13 示例 · 5 题）</summary>

- [Day09_列表生成式.py](刷题/Python100Days/Day09_列表生成式.py)
- [Day10_元组打包与解包.py](刷题/Python100Days/Day10_元组打包与解包.py)
- [Day11_字符串常用方法.py](刷题/Python100Days/Day11_字符串常用方法.py)
- [Day12_集合运算.py](刷题/Python100Days/Day12_集合运算.py)
- [Day13_字典的应用.py](刷题/Python100Days/Day13_字典的应用.py)
</details>

<details>
<summary>主题 4 · 函数（Day14-17 · 9 题）</summary>

- [Day14_函数参数.py](刷题/Python100Days/Day14_函数参数.py)
- [随机验证码.py](刷题/Python100Days/Day15_随机验证码.py)
- [判断素数.py](刷题/Python100Days/Day15_判断素数.py)
- [最大公约数和最小公倍数.py](刷题/Python100Days/Day15_最大公约数和最小公倍数.py)
- [数据统计.py](刷题/Python100Days/Day15_数据统计.py)
- [双色球随机选号.py](刷题/Python100Days/Day15_双色球随机选号.py)
- [Day16_高阶函数与Lambda.py](刷题/Python100Days/Day16_高阶函数与Lambda.py)
- [Day17_装饰器.py](刷题/Python100Days/Day17_装饰器.py)
- [Day17_递归.py](刷题/Python100Days/Day17_递归.py)
</details>

<details>
<summary>主题 5 · 文件操作（Day21 · 2 题）</summary>

- [Day21_读写文本文件.py](刷题/Python100Days/Day21_读写文本文件.py)
- [Day21_二进制文件拷贝.py](刷题/Python100Days/Day21_二进制文件拷贝.py)
</details>

<details>
<summary>主题 6 · 面向对象（Day18 / Day20 · 4 题）</summary>

- [时钟.py](刷题/Python100Days/Day18_时钟.py)
- [平面上的点.py](刷题/Python100Days/Day18_平面上的点.py)
- [扑克游戏.py](刷题/Python100Days/Day20_扑克游戏.py)
- [工资结算系统.py](刷题/Python100Days/Day20_工资结算系统.py)
</details>

<details>
<summary>主题 7 · 错误与异常（Day21 · 1 题）</summary>

- [Day21_自定义异常.py](刷题/Python100Days/Day21_自定义异常.py)
</details>

<details>
<summary>主题 8 · 模块与包（说明）</summary>

100-Days 中 Day14（函数和模块）已在主题 4 覆盖；Day22（对象序列化）为 JSON 知识讲解、无独立可运行题目，建议结合笔记学习后做自主练习。
</details>

<details>
<summary>主题 9 · Python 高级语法（说明）</summary>

- 装饰器 / 递归见主题 4（Day17_装饰器.py、Day17_递归.py）。
- 深浅拷贝、迭代器、生成器、闭包在 100-Days Day31 为知识提纲，无独立可运行题目，建议结合 `学习/9` 笔记练习。
</details>

<details>
<summary>主题 10 · 进程与线程（说明）</summary>

100-Days Day31「并发编程」为知识提纲（多线程 / 多进程 / 异步 I/O 概念与对比），无独立可运行题目，建议结合 `学习/10` 笔记学习。
</details>

<details>
<summary>主题 11 · 正则表达式（Day30 · 4 题）</summary>

- [验证输入用户名和QQ号是否有效并给出对应的提示信息.py](刷题/Python100Days/Day30_验证输入用户名和QQ号是否有效并给出对应的提示信息.py)
- [从一段文字中提取出国内手机号码.py](刷题/Python100Days/Day30_从一段文字中提取出国内手机号码.py)
- [替换字符串中的不良内容.py](刷题/Python100Days/Day30_替换字符串中的不良内容.py)
- [拆分长字符串.py](刷题/Python100Days/Day30_拆分长字符串.py)
</details>

### 自主做题

- [P01_1day.py](刷题/自主做题/P01_1day.py)（每天自主写 1 题，随学习进度持续追加）

---

## 五、如何运行

```bash
# 运行某个学习笔记 / 题目
python "学习/1 基础语法知识/day01_review.py"
python "刷题/59道基础入门题/python基础/计算BMI.py"
```

环境要求：Python 3.8+，无需第三方依赖。

---

## 六、维护说明

1. **每天学习后**：在「每日学习记录」表追加一行（日期、阶段、学习内容、练习/产出）。
2. **同步更新**：「30 天学习计划」中对应天数的状态由「待开始」改为「已完成」。
3. **提交推送**：

```bash
git add .
git commit -m "Day N：今日学习内容"
git push origin main
```
