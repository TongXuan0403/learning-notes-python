列表 list（有序、可变）
想干什么	                方法	                最小示例 & 易混点
长度 / 成员	        len(l)、x in l	        len([1,2])=2；3 in [1,3] 得 True
末尾加	            l.append(x)	            加一个整体：append([1,2]) → 嵌套
拼进来	            l.extend(iter)	        拆开逐个加：extend([1,2]) → 平铺
指定位置加	        l.insert(i,x)	        [1].insert(0,9) → [9,1]
删并拿到	            l.pop(i)	            默认删末尾，返回被删元素
按值删	            l.remove(x)	            只删第一个 x，没有就报错
找位置 / 次数	        l.index(x)、l.count(x)	index 找不到报错
求和 / 最值	        sum/max/min(l)	        元素必须是数字
原地排序	            l.sort(reverse=,key=)	直接改原表，返回 None
返回排序新表	        sorted(l,key=,reverse=)	原表不动，这是和 sort 的核心区别
推导式	            [expr for x in l if 条件]	用来生成新列表，别拿它做副作用


字符串 str（不可变 —— 所有方法都返回新串，原串不变）
想干什么	                方法	                    示例 & 易混点
找位置	                s.find(x)	            找不到返回 -1；index(x) 找不到会报错
数次数	                s.count(x)	            'hello'.count('l')=2
切开	                    s.split(sep)	        不写参数按任意空白切；写了按 sep 切，结果是列表
拼起来	                sep.join(列表)	        列表元素必须都是字符串：'-'.join(['2026','9'])
替换	                    s.replace(旧,新)	        返回新串，记得接住
去两端	                s.strip()	lstrip      只去左、rstrip 只去右
判类别	                isalpha/isdigit/isspace	字母 / 数字 / 空白，返回布尔
格式化	                f"{x:.2f}"	            产出字符串


字典 dict（键唯一）
想干什么	                    方法	                        易混点
取值	                        d[k]	                    键不存在报 KeyError
安全取值	                    d.get(k,默认)	            不存在返回默认值 / None，不报错
增 / 改	                    d[k]=v	                    有则改、无则增
不存在才加	                d.setdefault(k,默认)	        计数常用
合并	                        d.update(d2)	            同键覆盖
删	                        d.pop(k)、del d[k]	        pop 返回被删的值
遍历	                        d.items()	                for k,v in d.items() 同时拿键值
...
`&`交集 `|`并集 `-`差集 ；
去重 `list(set(l))`，
保序去重用 `list(dict.fromkeys(l))`。注意空集合是 `set()`，`{}` 是空字典。
